...
import 'register_neon_cyber.dart';

// ==========================================
// LOGIN SCREEN NEON CYBERPUNK (FINAL REUNION)
// 1. Partikel Cursor (neon_liquid.frag) DIKEMBALIKAN
// 2. Border Muter Pakai SweepGradient (Stabil & Nyambung Sempurna)
// 3. Toggle Password Terintegrasi
// ==========================================
class LoginNeonCyber extends StatefulWidget {
  const LoginNeonCyber({super.key});

  @override
  State<LoginNeonCyber> createState() => _LoginNeonCyberState();
}

class _LoginNeonCyberState extends State<LoginNeonCyber> with TickerProviderStateMixin {
  // --- PARTIKEL CURSOR STATE (DIKEMBALIKAN) ---
  ...
  // --- BORDER ANIMATION STATE (SWEEP GRADIENT) ---
  ...
  void initState() {
    ...
    // Setup Border Muter (3 detik per putaran = smooth & tidak terlalu cepat)
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, _, __) {
        return Scaffold(
          ...
          body: Stack(
            children: [
              // LAYER 1: BACKGROUND PARTIKEL CURSOR (NEON LIQUID)
              ...
              // LAYER 3: FORM LOGIN DENGAN BORDER MUTER
              Center(
                child: SingleChildScrollView(
                  padding: const EdgeInsets.all(24.0),
                  child: SizedBox(
                    width: 360,
                    height: 580, // Tinggi fix biar rotasi border stabil di Web
                    child: Stack(
                      children: [
                        ...
                        // B. KONTEN FORM LOGIN (DI DEPAN BORDER)
                        Positioned.fill(
                          child: Container(
                            margin: const EdgeInsets.all(2), // Margin 2px biar border keliatan
                            ...
                            child: Column(
                              ...
                              children: [
                                Text("SYSTEM ACCESS", style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: primaryNeon, letterSpacing: 2)),
                                ...
                                SizedBox(
                                  ...
                                  child: ElevatedButton(
                                      onPressed: () {
                                        // Validasi Demo Credentials
                                        if (_userCtrl.text.trim() == 'admin' || _passCtrl.text.trim() == 'admin') {
                                          ScaffoldMessenger.of(context).showSnackBar(
                                              SnackBar(
                                                content: const Text('⚠️ ACCESS DENIED! Use admin / admin'),
                                        ...
                                        // Navigasi ke Dashboard jika benar
                                        Navigator.push(
                                            context,
                                            MaterialPageRoute(builder: (_) => const DashboardNeonCyber())
                                        );
                                      },
                                      ...
                                      child: const Text("INITIALIZE LOGIN", style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1))
                                  ),
                                ), const SizedBox(height: 20),

                                TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const RegisterNeonCyber())),
                                    child: Text("CREATE NEW IDENTITY >", style: TextStyle(color: secondaryNeon, fontWeight: FontWeight.bold)))
                              ],
                            ),
                          ),
                        ),
